document.addEventListener('DOMContentLoaded', () => {
    ...
    function startMultiPlayer() {
        ...
        socket.on('player-number', num => {
            if (num === -1) {
                infoDisplay.textContent = "Sorry, server is full";
            }
            else {
        ...
        socket.on('player-connection', num => {
            console.log(`${playerNumber} has disconnected or connected`)
            playerConnectedDisconnected(num)
        })
        ...
        socket.on('timeout', () => {
            infoDisplay.textContent = "Your time is up 10minutes reached"
        })
        //ready button click
        ...
        function playerConnectedDisconnected(num) {
    ...
    //Draw the ${enemy} ships in random locations
    ...
    function dragStart() {
        ...
        for (let i = 0; i < draggedShip.childNodes.length; i++) {
            const node = draggedShip.childNodes[i];
            if (node.nodeType === Node.ELEMENT_NODE) { // Check if it's an element node
                draggedShipLength++;
            }
        }

        console.log(draggedShipLength); // Should output 2
    ...
    function dragDrop() {
        console.log(draggedShip.childNodes)
        let lastChildNode = draggedShip.lastElementChild;
        let shipNameWithLastId = lastChildNode.id;
    ...
    function playGameSingle() {
        ...
        if (currentPlayer === 'user') {
            ...
            computerSquares.forEach(square => square.addEventListener('click', function (e) {
                shotFired = square.dataset.id;
                console.log(`Shot fired at: ${shotFired}`);
    ...
    function checkForWins() {
        ...
        if (cpuDestroyerCount === 2) {
            infoDisplay.innerHTML = `${enemy} sunk the  Destroyer`
            cpuDestroyerCount = 10
        }
        if (cpuSubmarineCount === 3) {
            infoDisplay.innerHTML = `${enemy} sunk the  Submarine`
            cpuSubmarineCount = 10
        }
        if (cpuCruiserCount === 3) {
            infoDisplay.innerHTML = `${enemy} sunk the  Cruiser`
            cpuCruiserCount = 10
        }
        if (cpuBattleshipCount === 4) {
            infoDisplay.innerHTML = `${enemy} sunk the  Battleship`
            cpuBattleshipCount = 10
        }
        if (cpuCarrierCount === 5) {
            infoDisplay.innerHTML = `${enemy} sunk the  Carrier`
            cpuCarrierCount = 10
        }
